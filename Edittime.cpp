...
PropData Properties[] = {
	...
	PropData_Group(PROPID_GROUPRENDERING, (UINT_PTR)_T("Rendering"), (UINT_PTR)_T("Render settings.")),

		PropData_CheckBox(PROPID_AUTOREDRAW, (UINT_PTR)_T("Auto redraw"),		(UINT_PTR)_T("Automatically trigger \'On redraw\' every Fusion loop.")),

	PropData_Group(PROPID_GROUPDISPLAY, (UINT_PTR)_T("Display"), (UINT_PTR)_T("Surface display settings.")),

		PropData_Color(PROPID_TRANSPCOLOR, (UINT_PTR)_T("Transparent color"), (UINT_PTR)_T("The color that will not be displayed on the surface.")),
...
int WINAPI DLLExport CreateObject(mv _far *mV, fpLevObj loPtr, LPEDATA edPtr)
{
	...
	if ( IS_COMPATIBLE(mV) )
	{
		...
		edPtr->autoRedraw = true;
		edPtr->transpColor = RGB(0, 0, 0);
...
{
#ifndef RUN_ONLY
	edPtr->swidth = std::clamp<short>(cx, 1, 32767);
	edPtr->sheight = std::clamp<short>(cy, 1, 32767);
#endif // !defined(RUN_ONLY)
	return TRUE;	// OK
}
...
{
#ifndef RUN_ONLY
	rc->right = rc->left + edPtr->swidth;	// edPtr->swidth;
	rc->bottom = rc->top + edPtr->sheight;	// edPtr->sheight;
#endif // !defined(RUN_ONLY)
	return;
}
...
void WINAPI DLLExport EditorDisplay(mv _far *mV, fpObjInfo oiPtr, fpLevObj loPtr, LPEDATA edPtr, RECT FAR *rc)
{
	...
	if ( ps != NULL )		// Do the following if this surface exists
	{
		...
		if (GetSurfacePrototype(&proto, 32, ST_MEMORY, SD_DIB))
		{
			...
			auto bgColor = RGB(32, 32, 32);
			auto fgColor = RGB(128, 128, 128);
			...
			// Border
			// -----
			bg.Rectangle(0, 0, w, h, 1, fgColor);
			// -----

			bg.Blit(*ps, rc->left, rc->top, BMODE_TRANSP, BOP_BLEND, 70);
		}
		...
		int x = rc->left + (w / 2) - (is.GetWidth() / 2);	// get our boundaries
		int y = rc->top + (h / 2) - (is.GetHeight() / 2);

		is.Blit(*ps, x, y, BMODE_TRANSP, BOP_COPY);	// Blit the image to the frame editor surface!
